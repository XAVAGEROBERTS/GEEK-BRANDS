...
export const DataProvider = ({ children }) => {
  ...
  // ===== INITIAL LOAD (public reads — any client works) =====
  useEffect(() => {
    const loadAll = async () => {
      ...
      if (svcRes.error) console.error('Services fetch error:', svcRes.error);
  ...
  // ===== LOAD ORDERS (admin only — uses admin client) =====
  ...
  const addHeroSlide = async (slide) => {
    const { data, error } = await supabaseAdmin.from('hero_slides').insert([slide]).select().single();
    if (!error && data) setHeroSlides((prev) => [...prev, data]);
    return { data, error };
  };
  const updateHeroSlide = async (id, updates) => {
    const { data, error } = await supabaseAdmin.from('hero_slides').update(updates).eq('id', id).select().single();
  ...
  const addPortfolioItem = async (item) => {
    const { data, error } = await supabaseAdmin.from('portfolio').insert([item]).select().single();
  ...
  const addTeamMember = async (member) => {
    const { data, error } = await supabaseAdmin.from('team').insert([member]).select().single();
    if (!error && data) setTeam((prev) => [...prev, data]);
    return { data, error };
  };
  const updateTeamMember = async (id, updates) => {
    const { data, error } = await supabaseAdmin.from('team').update(updates).eq('id', id).select().single();
  ...
  const addOrder = async (order) => {
    // Public form submits to orders — use public client
